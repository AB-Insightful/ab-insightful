...
describe("isExperimentActive", () => {
  ...
  it("returns false for non-active statuses", () => {
    ...
    expect(
      isExperimentActive({ ...base, status: ExperimentStatus.draft }),
    ).toBe(false);
    expect(
      isExperimentActive({ ...base, status: ExperimentStatus.paused }),
    ).toBe(false);
    expect(
      isExperimentActive({ ...base, status: ExperimentStatus.completed }),
    ).toBe(false);
    expect(
      isExperimentActive({ ...base, status: ExperimentStatus.archived }),
    ).toBe(false);
  });
